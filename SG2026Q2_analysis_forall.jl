...
data_directory      = data_directories[4] ;
# Furnace 
const TCELSIUS = 205
...
jldopen(OUTFILE_PROCESSED, "r") do file
    ...
    if haskey(file["meta"], "SG1BfieldInTesla")
        ...
        fields   = file["meta"]["SG1BfieldInTesla"]

        # --- zero crossings: exact zeros or sign changes ---
        sign_changes = findall(1:length(fields)-1) do i
            fields[i] == 0 || fields[i] * fields[i+1] < 0
        end
        zero_currents = map(sign_changes) do i
            if fields[i] == 0
                currents[i]
            else
                I0, I1 = currents[i], currents[i+1]
        ...
        curr_neg  = abs.(currents[mask_neg])
        field_neg = abs.(fields[mask_neg])

        # --- linear plot ---
        fig_lin = plot(curr_pos, field_pos;
            line              = (:solid, 0.5, :blue),
            label             = false,
            marker            = (:circle, 2, :white),
        ...
        plot!(fig_lin, curr_neg, field_neg;
            line              = (:solid, 0.5, :red),
            label             = false,
            marker            = (:circle, 2, :white),
            markerstrokecolor = :red,
        ...
        # --- log-log plot ---
        log_xlims  = (0.5e-3, 1.1)
        log_xticks = ([1e-3, 1e-2, 1e-1, 1.0], [L"10^{-3}", L"10^{-2}", L"10^{-1}", L"10^{0}"])

        fig_log = plot(curr_pos, field_pos;
            line              = (:solid, 0.5, :blue),
            label             = false,
            marker            = (:circle, 2, :white),
            markerstrokecolor = :blue,
        ...
        plot!(fig_log, curr_neg, field_neg;
            line              = (:solid, 0.5, :red),
            label             = L"B_{z} < 0",
            marker            = (:circle, 2, :white),
            markerstrokecolor = :red,
        ...
        vline!(fig_log, abs.(zero_currents);
            line  = (:dash, 1.0, :black),
            label = L"B_z = 0",
        )

        # --- combined B-field layout ---
        fig = plot(fig_lin, fig_log;
            layout      = (2, 1),
            size        = (600, 800),
            left_margin = 3mm,
        )
    ...
    # --- read remaining metadata (available regardless of SG1 field key) ---
    AcqStep     = file["meta"]["AcquisitionStep"]
    Iexp_coil   = file["meta"]["SG1currentInA"]
    nI          = length(Iexp_coil)
    ...
    global AcqStep, Iexp_coil, nI
end

ΔIexp_coil     = 0.01 * Iexp_coil;
sort_Iexp_perm = sortperm(Iexp_coil);
...
jldopen(joinpath(DATA_SUMMARY_PATH, replace(data_directory * "_report_summary.jld2", "/" => "_")), "w") do file
    ...
    for (row, (nz,λ0)) in enumerate(PARAM_GRID)
        ...
        chosen_qm =  jldopen(data_qm_f1_path,"r") do file
            file[JLD2_MyTools.make_keypath_qm(nz, σw_um, 0.001)]
        end
        ...
        pretty_table(df_mean;
            title        = "Mean analysis",
            formatters   = [fmt__printf("%d", [1]), fmt__printf("%8.5f", 2:3), fmt__printf("%8.3f", 4:10)],
            alignment    = :c,
            column_labels = [
                ["Step", "Current","Current Error","F1 z","F2 z","Δz","Centroid F1 z","Centroid F1 z Error","Centroid F2 z","Centroid F2 z Error"],
            ...
            highlighters = [
                TextHighlighter((d,i,j) -> d[i,2] == minimum(d[:,1]), crayon"fg:white bold bg:dark_gray"),
                TextHighlighter((d,i,j) -> d[i,7] < 0,                crayon"fg:red bold bg:dark_gray"),
                TextHighlighter((d,i,j) -> d[i,9] > 0,                crayon"fg:green bold bg:dark_gray"),
            ],
        )
        ...
        pretty_table(df_fw;
            ...
            formatters   = [fmt__printf("%d", [1]), fmt__printf("%8.5f", 2:3), fmt__printf("%8.3f", 4:13)],
            alignment    = :c,
            column_labels = [
                ["Step", "Current","Current Error","F1 z","Std.Err.","F2 z","Std.Err.","Δz","Std.Err.","Centroid F1 z","Std.Err.","Centroid F2 z","Std.Err."],
            ...
            highlighters = [
                TextHighlighter((d,i,j) -> d[i,2] == minimum(d[:,1]), crayon"fg:white bold bg:dark_gray"),
                TextHighlighter((d,i,j) -> d[i,10] < 0,                crayon"fg:red bold bg:dark_gray"),
                TextHighlighter((d,i,j) -> d[i,12] > 0,               crayon"fg:green bold bg:dark_gray"),
            ],
        )
        ...
        ***************************************************
        ...
        file[JLD2_MyTools.make_keypath_exp(data_directory, nz, λ0)] = OrderedDict(
            ...
            # Mean profiles
            :F1_profile             => profiles_F1.mean[sort_Iexp_perm,:],
            :F1_err                 => profiles_F1.sem[sort_Iexp_perm,:],
            :F1_profile_spline      => hcat(f1_mean_spline_profile[:,1], f1_mean_spline_profile[:,1 .+ sort_Iexp_perm]),
            :F2_profile             => profiles_F2.mean[sort_Iexp_perm,:],
            :F2_err                 => profiles_F2.sem[sort_Iexp_perm,:],
            :F2_profile_spline      => hcat(f2_mean_spline_profile[:,1], f2_mean_spline_profile[:,1 .+ sort_Iexp_perm]),
