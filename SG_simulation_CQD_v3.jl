...
for (i, ki) in enumerate(kis)
    ...
    tmp_up_flags = tmp_up_traj = nothing   # free before building down
    ...
    tmp_up_screen = tmp_mm_up = nothing
    GC.gc()
    ...
    tmp_dw_flags = tmp_dw_traj = nothing
    ...
    # Explicitly free memory before the next iteration
    tmp_dw_screen = tmp_mm_dw = nothing
    GC.gc()
end
...
function process_branch(branch::Symbol, indir, outjld, files, kis, params)
    ...
    for (j, fname) in pairs(files)
        ...
        fill!(local_chunks, [])    # free profile data immediately after write
