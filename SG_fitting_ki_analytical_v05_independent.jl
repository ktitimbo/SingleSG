...
#  as a function of coil current I. CQD predictions exist only on a discrete
#  grid of (current, kᵢ); this script builds a smooth 2D interpolant over that
#  grid, fits kᵢ against the experimental curve, and compares the result with
#  the quantum-mechanical (QM) reference curve.
#
#  HIGH-LEVEL WORKFLOW
#  -------------------
#    0. Setup: packages, output directory, local modules.
...
#    4. Fit kᵢ by minimising a log10-space residual between ki_itp and data,
#       with/without a magnification (scale) correction.
#    5. Diagnostics & figures: fit curves, relative errors, residuals,
#       goodness-of-fit tables; everything written to OUTDIR.
#
#  ANALYSIS PARTS (run top-to-bottom in one Julia session)
#  --------------------------------------------------------
#    PART 1 — Combined curve, QM-only scaling.
#             Scale factor from the QM tail alone, then kᵢ fitted on original
#             and on scaled data (`fit_ki`, `fit_ki_with_error`).      fig004–005
#    PART 2 — Batch analysis of each dated experimental dataset
#             (`fit_k_parameter`, `fit_ki_with_error`, summary plots).
#    PART 3 — Joint (scale + kᵢ) fit on the combined curve
#             (`fit_ki_joint_scaling_fitsubset`), publication figures,
#             CSV exports, goodness-of-fit table.                       fig006–007
#
...
#         simulation_data/QM_T205_8M/qm_screen_profiles_f1_table.jld2
#         simulation_data/cqd_simulation_7M/cqd_7000000_up_profiles_bykey.jld2
#         EXPDATA_ANALYSIS/smoothing_binning_2025/data_averaged_2.jld2
#         EXPDATA_ANALYSIS/<date>/…/fw_data.csv   (batch loop, PART 2)
#
#  OUTPUTS  (OUTDIR = ./data_studies/FITki<timestamp>/)
#  ----------------------------------------------------
#    fig001–fig007.<FIG_EXT>, single_SG_comparison(.png|.svg),
#    single_SG_comparison_vsg(.png|.svg), rel_error_{original,scaled}.csv,
#    data_exp.csv, data_sim.csv; tables and fit summaries printed to stdout.
#
#  CONVENTIONS
#  -----------
#    • kᵢ is stored in "micro-units": physical value = (reported kᵢ) × 10⁻⁶.
#    • Experimental data matrices are N×4 with columns [I, δI, z, δz]
#      (I in A, z in mm). Column 2 (δI) is a 2 %-of-I placeholder throughout.
#    • All kᵢ fits minimise a residual in log10(z); reported "errors" are
#      documented per function (RMSE in mm, t-interval on kᵢ, …).
#
#  CHANGES vs v04
#  --------------
#    • Removed exploratory scratch block (xukun/suley comparison, BvsI call),
#      the unused `keypath` helper, unused package imports, dead commented code
#      and a discarded `fit_ki` call in PART 3.
#    • Docstrings moved *before* each `function` (inside the body they were
#      inert string literals, not documentation).
#    • Numerical settings, data paths and figure outputs are unchanged.
# =============================================================================
...
Fit the induction coefficient `kᵢ` by minimising a mean-squared error in
**log10 space** between the interpolated prediction `ki_itp(I, kᵢ)` and a
selected subset of data points. The fit is therefore sensitive to *relative*
deviations across orders of magnitude.
 
Although the optimisation is done in log space, the reported error is the
root-mean-square error in **linear space** (same units as `z`, i.e. mm).
 
# Arguments
- `data_org`        : N×4 matrix `[I, δI, z, δz]`. Full data set, used only to
                      compute R² in linear space (columns 1 and 3).
- `selected_points` : same column convention; the subset actually fitted.
                      All `z` (column 3) must be strictly positive.
- `ki_list`         : vector of candidate `kᵢ` values (micro-units).
- `ki_range`        : `(ki_start, ki_stop)` indices into `ki_list` giving the
                      Brent search bracket.
 
# Returns
NamedTuple `(ki, ki_err, r2_coeff)`:
- `ki`       : best-fit `kᵢ`
- `ki_err`   : linear-space RMSE (mm) on `selected_points` — a goodness-of-fit
               number, **not** an uncertainty on `kᵢ`
- `r2_coeff` : R² in linear space on `data_org`
 
# Notes
- Uses the global 2D interpolant `ki_itp` (must be defined before calling).
- For a formal uncertainty on `kᵢ` use `fit_ki_with_error`.
"""
...
Two-parameter fit of the induction coefficient `kᵢ` and a global calibration
(magnification) factor `s`, defined so that the **scaled CQD model** `m/s`
matches the raw experiment `z`.

The scale is tied to the high-current tail: for a candidate `kᵢ`, `s(kᵢ)` is
the least-squares factor that makes `itp(I,kᵢ)/s` agree with the last `n_tail`
rows of `data_org`; `kᵢ` is then chosen by minimising the log10-space MSE
between `z` and `itp(I,kᵢ)/s(kᵢ)` on `selected_points` (bounded Brent search).
Because `s(kᵢ)` is closed-form, the joint (kᵢ, s) problem reduces to a 1D
search — the returned pair is the joint optimum of the composite objective.

# Arguments
- `data_org`        : N×4 matrix `[I, δI, z, δz]`, full data set. Its last
                      `n_tail` rows define the calibration; all rows are used for R².
- `selected_points` : same layout; the subset used for the kᵢ loss.
- `ki_list`, `ki_range` : as in `fit_ki` (bracket `ki_list[ki_start:ki_stop]`).
- `n_tail`          : number of tail points of `data_org` used for the scale.
- `itp`             : callable `(I, kᵢ) -> z_model` (default: global `ki_itp`).
- `scale_mode`      : `:linear` → `s = ⟨m, m⟩ / ⟨z, m⟩`, least squares in mm,
                      minimises Σ(z − m/s)² (residuals in data space);
                      `:log` → `log10 s = mean(log10 m − log10 z)`, least squares
                      in log space (geometric-mean ratio; consistent with the
                      log-space kᵢ loss);
                      `:legacy` → `s = ⟨m, z⟩ / ⟨z, z⟩`, minimises Σ(m − s·z)²
                      (residuals in model space; = 1/`scaled_mag` of v04).

# Returns
NamedTuple:
- `ki`, `scale`     : best-fit pair; `m/scale` overlays the raw data.
                      `scale_inv = 1/scale` is the factor that multiplies the
                      *data* to reach the model (old convention).
...
function fit_cqd_ki_scale(data_org, selected_points, ki_list, ki_range;
    ...
    I_all = data_org[:, 1]
    z_all = data_org[:, 3]
    p_all = itp.(I_all, Ref(k_fit)) ./ s_fit
...
parameter, so this is the whole fit).

`scale_mode` (same conventions as `fit_cqd_ki_scale`):
- `:linear` → `s = ⟨m, m⟩ / ⟨z, m⟩`   minimises Σ(z − m/s)²  (data-space residuals)
- `:log`    → `log10 s = mean(log10 m − log10 z)`   least squares in log space
- `:legacy` → `s = ⟨m, z⟩ / ⟨z, z⟩`   minimises Σ(m − s·z)² (model-space residuals;
              = 1/`scaled_mag` of v04)
...
Fit `kᵢ` in **log10 space** and return a formal uncertainty on the fitted
value — the uncertainty-aware companion of `fit_ki`.
 
The fit minimises the (optionally weighted) residual sum of squares
`RSS(kᵢ) = Σ wᵢ (log10 itp(Iᵢ,kᵢ) − log10 Zᵢ)²` with a bounded Brent search.
With `use_Zse=true` the weights are the propagated log-space inverse variances
`wᵢ = (Zᵢ ln10 / σZᵢ)²`; otherwise all points are weighted equally.
 
Uncertainty is reported two independent ways:
- Linearised standard error `se` from `Var(k̂) ≈ σ² / (JᵀWJ)`, with the
  Jacobian `dr/dk` from central finite differences, giving the Student-t
  interval `ci_t` at confidence `conf`.
- A likelihood-profile interval `ci_profile`: the loss is walked away from
  `k̂` until `RSS` rises by `Δ = χ²₁(conf)` (multiplied by `σ²` when the fit
  is unweighted) and the crossing is located by bisection.
 
# Arguments
- `itp`          : callable `(I, kᵢ) -> z_pred` (the CQD interpolant `ki_itp`).
- `data`         : N×4 matrix `[I, δI, Z, σZ]`; `σZ` used only if `use_Zse`.
- `bounds`       : `(ki_min, ki_max)` search bracket.
- `conf`         : confidence level of the intervals.
- `use_Zse`      : weight by measurement uncertainties.
- `profile`      : also compute the profile interval.
- `profile_grid` : grid points used to bracket the profile crossing.
 
# Returns
NamedTuple with `ki`, `ki_err` (= t-critical × `se`), `se`, `ci_t`,
`ci_profile`, `delta_target`, `delta_rss`, `profile_note`, `rss`, `sigma2`,
`dof`, `n_used`, `r2_coeff` (weighted R² in log10 space), `converged` and the
raw Optim `result`.
"""
...
Uncertainty on `kᵢ` (as in `fit_ki_with_error`, but on the composite residuals
so that `dr/dk` includes the tail-driven `ds/dk`):
...
Uncertainty on `s` (reported in `u = log10 s`, then mapped back):
`Var(u) = Var(u | kᵢ)_tail + (du/dk)² Var(k̂)`, where the first term is the
sampling variance of the tail estimator at fixed `kᵢ` (scatter of the tail
residuals / `n_tail`; `1/Σw` for the weighted `:log` mode) and the second is
propagated from `kᵢ`. `scale_ci_t` is the t-interval `s·10^(±t·se_u)`
(asymmetric in `s`), `scale_ci_from_profile` maps `ci_profile` through `s(kᵢ)`
(propagation only, no tail scatter), and `corr_ki_scale` is the correlation
between `k̂` and `û` induced by the tail (`du/dk · Var(k̂) / (se·se_u)`).
...
- `bounds`          : `(ki_min, ki_max)` search bracket.
- `scale_mode`      : `:log` → `log10 s = ⟨log10 m − log10 z⟩_tail` (weighted if
                      `use_Zse`); `:linear` → `s = ⟨m,m⟩/⟨z,m⟩`; `:legacy` →
                      `s = ⟨m,z⟩/⟨z,z⟩`. The linear modes ignore `use_Zse` for
                      the scale and use the regression residual variance instead.
- `conf`, `use_Zse`, `profile`, `profile_grid` : as in `fit_ki_with_error`.

# Returns
NamedTuple with
- kᵢ block   : `ki`, `ki_err`, `se`, `ci_t`, `ci_profile`, `delta_target`,
               `delta_rss`, `profile_note`, `rss`, `sigma2`, `dof`, `n_used`,
               `r2_coeff` (weighted R² in log10 space), `converged`, `result`;
...
function fit_cqd_ki_scale_with_error(itp, data_org, selected_points;
    ...
    # ---------------- scale s(kᵢ) and its conditional variance ----------------
    # returns (s, Var(log10 s | kᵢ)) from the tail alone
    function scale_for(ki)
        m  = itp.(I_t, Ref(ki))
        ok = isfinite.(m) .& (m .> 0)
    ...
    # R² in log10 space (weighted) on mJ
    y   = log10.(Z[mJ])
    ŷ   = y .+ rJ
    ȳw  = sum(wJ .* y) / sum(wJ)
    TSS = sum(wJ .* (y .- ȳw) .^ 2)
...
Estimator (`scale_mode`, same conventions as `fit_cqd_ki_scale_with_error`):
- `:log`    → `u = log10 s = ⟨log10 m − log10 z⟩_tail` (weighted mean if
              `use_Zse`); `Var(u) = var(d)/n` (or `1/Σw`).
- `:linear` → `z ≈ a·m`, `s = 1/a`, `a = ⟨z,m⟩/⟨m,m⟩`; `Var(a)` from the
              regression residuals, mapped to `Var(u)`.
- `:legacy` → `m ≈ s·z`, `s = ⟨m,z⟩/⟨z,z⟩`; `Var(s)` from the residuals.
The linear modes ignore `use_Zse` for the estimator.

Uncertainty is reported two ways, as for `kᵢ`:
- Student-t interval `scale_ci_t = s·10^(±t·se_u)` at confidence `conf`
  (symmetric in `log10 s`, hence slightly asymmetric in `s`), with
  `scale_se = s·ln10·se_u` and `scale_err = t·scale_se`.
- Profile interval `scale_ci_profile`: the log10-space weighted RSS on the tail,
  `RSS(u) = Σ wᵢ (log10 mᵢ − u − log10 zᵢ)²`, is walked away from `û` until it
  rises by `χ²₁(conf)` (× `σ²` if unweighted) and the crossing is bisected.
  (`RSS(u)` is exactly quadratic in `u`, so this mostly serves as a
  consistency check of the t-interval.)
...
function fit_qm_scale_with_error(zqm, data_org;
    ...
    # log10 ratios model/data on the tail
    d = log10.(m_t) .- log10.(Z_t)
    ...
    # ---------------- log10-space RSS in u (for σ², dof and the profile) ----------------
    ...
    if profile
        ...
        target = RSS0 + Δrss
        # search window: generous multiple of the t-interval half-width
        halfw = max(10 * tcrit * se_u, 1e-6)
        u_min, u_max = û - halfw, û + halfw

        function bracket_side(dir::Int)
            grid  = range(û, dir > 0 ? u_max : u_min; length=profile_grid)
            prevu = first(grid)
            prevL = loss(prevu)
            for u in Iterators.drop(grid, 1)
                L = loss(u)
                if isfinite(L) && (L > target) && isfinite(prevL) && (prevL <= target)
                    return (prevu, u)
                end
        ...
        function bisect_cross(a, b; maxiter=80, tol=1e-12)
            lo, hi = a, b
            for _ in 1:maxiter
                mid  = (lo + hi) / 2
                fmid = loss(mid) - target
        ...
        left_br  = bracket_side(-1)
        right_br = bracket_side(+1)
        u_lo = left_br  === nothing ? u_min : bisect_cross(left_br[1],  left_br[2])
        u_hi = right_br === nothing ? u_max : bisect_cross(right_br[1], right_br[2])
