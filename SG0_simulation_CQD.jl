...
const OUTDIR    = joinpath(@__DIR__, "simulation_data", "SG0_" * RUN_STAMP);
...
# Initial angles: generated once
θe0 = Vector{Float64}(undef, Nss)
θn0 = Vector{Float64}(undef, Nss)

@inbounds @simd for i in eachindex(θe0)
    r1 = rand(rng_set)
    r2 = rand(rng_set)
    θe0[i] = 2asin(sqrt(r1))
    θn0[i] = 2asin(sqrt(r2))
...
#   COQUANTUM DYNAMICS
##################################################################################################
results = OrderedDict{Float64, OrderedDict{Symbol, NamedTuple}}()
for (B0_mT, B0) in zip(B0_list_mT, B0_list)
    @info "Running B0 = $(B0_mT) mT"

    θeB = Vector{Float64}(undef, Nss)
    @inbounds @simd for i in eachindex(θeB)
        θeB[i] = 2 * atan(
            tan(θe0[i] / 2) *
            exp(-sign(θn0[i] - θe0[i]) * kI * abs(γₑ) * B0 * L0 / crossing_slit[i, 5])
        )
    end
    ...
    cases = (
        up = hcat(crossing_slit[.!mask, :], θeB[.!mask], θn0[.!mask]),
        dw = hcat(crossing_slit[ mask, :],  θeB[ mask],  θn0[ mask]),
    )

    branch_results = OrderedDict{Symbol, NamedTuple}()

    for (branch_key, data_mat) in pairs(cases)
        branch_tag = branch_key === :up ? :up : :down

        @time flag = TheoreticalSimulation.CQD_flag_travelling_particles(
            Icoils, data_mat, kI, K39_params;
            y_length=1001, verbose=true
        )

        @time traj = TheoreticalSimulation.CQD_build_travelling_particles(
            Icoils, kI, data_mat, flag, K39_params
        )

        TheoreticalSimulation.CQD_travelling_particles_summary(
            Icoils, traj, branch_tag
        )

        screen = OrderedDict(
            :Icoils => Icoils,
            :data   => TheoreticalSimulation.CQD_select_flagged(traj, :screen)
        )

        mm = TheoreticalSimulation.CQD_analyze_profiles_to_dict(
            screen;
            n_bins      = (nx_bins, nz_bins),
            width_mm    = gaussian_width_mm,
            add_plot    = false,
            plot_xrange = :all,
            branch      = branch_key,
            λ_raw       = λ0_raw,
        ...
        branch_results[branch_key] = (screen=screen, mm=mm)
    end

    results[B0_mT] = branch_results
end


ni_chosen = 17
dup = [results[v][:up].mm[ni_chosen][:z_max_smooth_spline_mm] for v in B0_list_mT]
ddw = [results[v][:dw].mm[ni_chosen][:z_max_smooth_spline_mm] for v in B0_list_mT]
Δz = dup .- ddw

fig01 = plot(B0_list_mT, dup,
    label="CQD up",
...
plot!(B0_list_mT, ddw,
    ...
    line=:blue)
plot!(xlabel=L"SG0: $B_{0}$ (mT)",
    ylabel=L"$z$ position (mm)",
    legend=:right,
    legend_title = "$(Int(1000*Icoils[ni_chosen]))mA",
    foreground_color_legend = nothing,
    background_color_legend = nothing,)

fig02 = plot(B0_list_mT, Δz./6.5e-3,
label="Splitting",
...
#   Opposite polarity
##################################################################################################
results_opposite = OrderedDict{Float64, OrderedDict{Symbol, NamedTuple}}()
for (B0_mT, B0) in zip(B0_list_mT, B0_list)
    @info "Running B0 = $(B0_mT) mT"
    ...
    θeB_flip = theta_wrap.(θeB .+ pi)
    θn_flip = theta_wrap.(θn0 .+ pi)

    mask = θeB_flip .> θn_flip

    cases = (
        up = hcat(crossing_slit[.!mask, :], θeB_flip[.!mask], θn_flip[.!mask]),
        dw = hcat(crossing_slit[ mask, :],  θeB_flip[ mask],  θn_flip[ mask]),
    )
    ...
    results_opposite[B0_mT] = branch_results
end


ni_chosen = 17
dup = [results_opposite[v][:up].mm[ni_chosen][:z_max_smooth_spline_mm] for v in B0_list_mT]
ddw = [results_opposite[v][:dw].mm[ni_chosen][:z_max_smooth_spline_mm] for v in B0_list_mT]
Δz = dup .- ddw
