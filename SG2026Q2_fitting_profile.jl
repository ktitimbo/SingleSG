...
function run_analysis(wanted_zbinning::Integer, wanted_smooth::Real; save_results::Bool=false)
    ...
    if save_results
        jldopen(joinpath(OUTDIR, "blur_conv_$(P_DEGREE).jld2"), "w") do f
    ...
    ***************************************************
    ...
    if save_results
        open(joinpath(OUTDIR, "convolution_report_$(P_DEGREE).txt"), "w") do io
            write(io, report)
        end
    end
...
for (zbinning, smoothing) in Iterators.product(ZBINNING_LIST, SMOOTH_LIST)
    ...
    inspection_results[(zbinning, smoothing)] = run_analysis(
        zbinning,
        smoothing;
