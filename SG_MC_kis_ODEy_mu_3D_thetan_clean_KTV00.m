...
% kis = 10.^[linspace(-5.2,-5.2,1)]
kis = 2.0e-6 %10.^[linspace(-5.6,-5.6,1)]
...
Icoils = [0.0, 0.002, 0.004, 0.006, 0.008, 0.010, 0.012, .016, 0.022, 0.032, 0.044, 0.058,...
     0.074, 0.094, 0.114,  0.134,  0.154,  0.174,  0.195,  0.215,  0.236,  0.26,...
     0.285,  0.308,  0.341,  0.381,  0.42,  0.46,  0.5,  0.55,  0.609, 0.75, 1.0];
...
T = 473; % K
ps.M = 39.0983 / (6.02e23 * 1000); % kg
ps.uB = 9.274e-24;  % J / Tesla
...
% Slit thicknesses
ws = 0.1e-3; % furnace slit width z
ls = 2e-3; % furnace slit length x
wSG1 = 0.3e-3; % SG1 slit width z
...
ps.dBzdz = interp1(gradientvscurrent(:,1),gradientvscurrent(:,2),coilcurrent);
...
% Concatanate initial values for ode variables
...
dvz(1,:) = (1./ps.vy') .* (- ps.m_signs' .* ps.uB * dBzdz_y / ps.M .* cos( 2*atan( tan(ps.theta_e'/2).*exp(ps.ki*u(3,:)) ) ) );
