...
function SG0_stack_data(data_directory::AbstractString;
                    ...
                    error_factor    = 0.015,
                    keynames        = ("BG","F1","F2"),
                    key_MGcurrent   = "MGcurrentinA",     # key name in .mat file
                    key_MGfield     = "MGfieldinGauss",     # key name in .mat file
                    verbose::Bool   = true
)
    ...
    tokens_sg1 = Vector{SubString{String}}(undef, Nfiles)
    for (i, f) in enumerate(files)
        m = match(SG1Curr, f)
        @assert m !== nothing "Couldn't parse current from: $f"
    ...
    tokens_sg0 = Vector{SubString{String}}(undef, Nfiles)
    for (i, f) in enumerate(files)
        m = match(SG0Curr, f)
        @assert m !== nothing "Couldn't parse current from: $f"
    ...
    # 2c) Extract ampmeter range tokens (for current uncertainties)
    ran_tokens = Vector{SubString{String}}(undef, Nfiles)
    for (i, f) in enumerate(files)
        mr = match(ampmeter_scale, f)
        @assert mr !== nothing "Couldn't parse range (Ran...A) from: $f"
        ran_tokens[i] = mr.captures[1]  # e.g. "10m", "1000m", "500u"
    ...
    tokens_bz1 = Vector{SubString{String}}(undef, Nfiles)
    for (i, f) in enumerate(files)
        m = match(SG1Bz, f)
        @assert m !== nothing "Couldn't parse current from: $f"
    ...
    tokens_bz0 = Vector{SubString{String}}(undef, Nfiles)
    for (i, f) in enumerate(files)
        m = match(SG0Bz, f)
        @assert m !== nothing "Couldn't parse current from: $f"
    ...
    # 2f) Extract step index (fallback to file index if absent)
    Nstep = collect(1:Nfiles)    # default: sequential
    for (i, f) in enumerate(files)
        m = match(step_count, f)
        m !== nothing && (Nstep[i] = parse(Int, m.captures[1]))
    end

    # Convert tokens to physical units
    currents_SG0    = parse_amp.(tokens_sg0)
    currents_SG1    = parse_amp.(tokens_sg1)
    ran_vals        = parse_amp.(ran_tokens)
    ...
    F1 = Array{T}(undef, sz[1], sz[2], sz[3], Nfiles)
    F2 = similar(F1)
    BG = similar(F1)
    currents_MG = zeros(Float64, Nfiles)     # default: zero; overwritten if key present
    fields_MG = zeros(Float64, Nfiles)
    ...
    for (i, f) in enumerate(files)
        matopen(f) do fh
            ...
            # Read MG current if the key exists, otherwise keep 0.0
            if haskey(fh, key_MGcurrent)
                currents_MG[i] = Float64(read(fh, key_MGcurrent))
            else
                verbose && @warn "Key '$key_MGcurrent' not found in $(basename(f)) — defaulting to 0 A"
            end

            if haskey(fh, key_MGfield)
                fields_MG[i] = Float64(read(fh, key_MGfield))
            else
                verbose && @warn "Key '$key_MGfield' not found in $(basename(f)) — defaulting to 0 G"
            end
        end
        verbose && i % 5 == 0 && @info "Loaded $i / $Nfiles" file=f
    end
    ...
    out = OrderedDict(
        ...
        :Files          => files,
        :Step           => Nstep, 
        :MGCurrents     => currents_MG,         # A  (0.0 if absent in .mat)
        :MGFields       => fields_MG .* 1e-4,   # T (0.0 if absent in .mat)
...
const _scipy_io_ref = Ref{Any}(nothing)

function _get_scipy_io()
    if _scipy_io_ref[] === nothing
        _scipy_io_ref[] = pyimport("scipy.io")
    end
    return _scipy_io_ref[]
end
...
function SG0_build_processed_dict(raw_data::OrderedDict{Symbol,Any};
    ...
    out = OrderedDict(
        ...
        :MGCurrents     => raw_data[:MGCurrents],         # Guiding field current
        :MGFields       => raw_data[:MGFields],           # teslas (0.0 if absent in .mat)
...
function SG0_framewise_maxima(signal_key::String, data, nz_bin::Integer; 
    ...
    for j in 1:nI0
        ...
        for i in 1:n_frames
            ...
            # # --- Maxima via scalar bounded optimization
            # zlo, zhi = extrema(z_fit)

            # negative_spline(z) = -S_fit(z)

            # res = optimize(negative_spline, zlo, zhi, Brent())

            # if !Optim.converged(res)
            #     @warn "Spline maximum optimization did not converge" frame=i current_index=j res=res
            # end

            # max_z = Optim.minimizer(res)
