module MyPolylogarithms

    using SpecialFunctions  # Use any necessary packages, such as SpecialFunctions
    ...
    # Diagnostics struct for the optional diagnostics output
    ...
    * `series`:    The series used to compute results (4 = reciprocal)
    ...
    #    elseif t <= 2.0
            ...
            # println("  main level $level, z=$z,  abs(μ)/2π = ", abs(log(z))/2π  )
        ...
        # else
        #     throw(DomainError(z, "bad z value $z with |z|=$(abs(z)) and |log(z)/(2π)|=$(abs(μ/2π))"))
        end
    ...
    # old version without square root
        # if abs(z) <= 0.5 && abs(z) < abs(μ/2π)
        #     return polylog_series_1(s, z; accuracy=accuracy, min_iterations=min_iterations, max_iterations=max_iterations)
        # elseif abs(μ/2π) < series2_threshold && ( abs(round(real(s))-s) > tau_threshold || real(s)<= 0 )
        #     return polylog_series_2(s, z; accuracy=accuracy, min_iterations=min_iterations, max_iterations=max_iterations)
        # elseif abs(μ/2π) < series2_threshold
        #     return polylog_series_3(s, z; accuracy=accuracy, min_iterations=min_iterations, max_iterations=max_iterations)
        # elseif abs(z) > 1
        #     return polylog_reciprocal(s, z; accuracy=accuracy, min_iterations=min_iterations, max_iterations=max_iterations)
        # else
    ...
    #     x = im * (log(convert(Complex{Float64}, -z)) / 2π)
    #     ss = 1-s
    #     ip = im^ss
    #     return ( SpecialFunctions.gamma(ss)/2π^(ss) ) * (ip * SpecialFunctions.zeta(ss, 0.5+x) + conj(ip) * SpecialFunctions.zeta(ss, 0.5-x))
    # end
    ...
    function polylog_duplication(s::Number, z::Number;
        ...
        # println("  dup level $level, z=$z,  abs(μ)/2π = ", abs(log(z))/2π  )
    ...
    #         A = 0.0
    #     else
    #         # G = (2π*im)^s * SpecialFunctions.zeta( 1-s, 0.5 + log(complex(-z))/(2π*im) ) /  SpecialFunctions.gamma(s)
    #         # A = 2π*im*log(z)^(s-1) / SpecialFunctions.gamma(s)
    #         tmp = exp( s*log(2π*im) - SpecialFunctions.loggamma(complex(s)) )  # (2π*im)^s /  SpecialFunctions.gamma(s)
    #         G = tmp * SpecialFunctions.zeta( 1-s, 0.5 + log(complex(-z))/(2π*im) ) 
    #         A = 2π*im*log(z)^(s-1) / SpecialFunctions.gamma(s)
    #     end
    ...
    function polylog_series_2(s::Number, z::Number;
        ...
        # else
            # min_iterations = ceil( -real(s) ) + 1
            # min_iterations = ceil( real(s) / log(abs( log(z)/2π ) ) )
        # end
        oneminuss = 1.0 - s
        ...
        a = Inf
        a_2 = Inf
        # A = abs( 2.0*2π^real(s) * exp(abs(imag(π*s)))  )
        ...
        while k<=max_iterations && ~converged
            ...
            if k > min_iterations &&
                ...
                abs(a_2) > abs(a)
                # abs( A * (k-real(s))^real(-s) * (μ/2π)^k )/abs(total) < accuracy
                # && abs( 2*2π^real(s) * (μ/2π)^k )/abs(total) < accuracy 
    ...
    function polylog_series_3(s::Number, z::Number;
        ...
        while k<=max_iterations && ~converged
            ...
            if k > min_iterations &&
                ...
                abs(a_2)/abs(total) < 0.5*accuracy &&
                abs(a_2) > abs(a)
                # abs( (μ/2π)^k )/abs(total) < 0.05*accuracy
                converged = true
            end
            k = k + 1
        end
    ...
    #     while k<=max_iterations && ~converged
    #         old_a = a
    #         a = (-1)^k * (1.0 - 2.0^(1-2*k)) * 2π^(2*k) * bernoulli(2*k,0.0) * x^(s-2*k) / ( SpecialFunctions.gamma(2*k+1) * SpecialFunctions.gamma(s+1-2*k) )
    #         if abs(a) < abs(old_a)
    #             total += a
    #         else
    #             converged = true
