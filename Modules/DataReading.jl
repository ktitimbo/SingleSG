...
"""
    vector_subset(v::AbstractVector{T}; thr::Real = zero(T), include_equal::Bool = true) where {T<:Real}
        -> (mask, inds, values_view)

Select elements of a real-valued vector `v` that satisfy a threshold condition,
and return the corresponding boolean mask, indices, and filtered view.

By default the function selects entries satisfying

    v[i] ≥ thr

If `include_equal=false`, the strict condition

    v[i] > thr

is used instead.

This helper is useful when constructing a selection mask from a reference
vector and then applying that mask to other aligned arrays (e.g. positions,
uncertainties, weights).

Arguments
---------
v::AbstractVector{T}
    Input vector with real-valued element type.

thr::Real = zero(T)
    Threshold used for the comparison. The value is converted internally
    to type `T`.

include_equal::Bool = true
    If true the condition `≥` is used, otherwise the strict `>` comparison.
...
mask
    Boolean vector of length `length(v)` identifying elements satisfying
    the threshold condition.

inds
    Vector of integer indices `i` such that `mask[i] == true`.

values_view
    View of the selected elements `@view v[mask]`.

Notes
-----
The threshold is internally converted with

    thrT = convert(T, thr)

to ensure comparisons are performed in the same numeric type as `v`.

Examples
--------
v = [-0.2, 0.0, 0.3, 0.8]

mask, inds, vals = vector_subset(v; thr=0.0)

mask, inds, vals = vector_subset(v; thr=0.0, include_equal=false)
"""
...
Select rows of a real-valued matrix `A` for which all specified columns
satisfy a threshold condition.

A row `i` is selected if every column `c` in `cols` satisfies

    A[i,c] ≥ thr

when `include_equal=true` (default), or

    A[i,c] > thr

when `include_equal=false`.

Arguments
---------
A::AbstractMatrix{T}
    Input matrix with real-valued element type.

cols::AbstractVector{<:Integer}
    Column indices on which the threshold condition is tested.

thr::Real = zero(T)
    Threshold value used in the comparison. Internally converted to `T`.

include_equal::Bool = true
    If true use the comparison `≥`, otherwise use the strict `>`.
...
mask
    Boolean vector of length `size(A,1)` identifying rows satisfying the
    condition across all selected columns.

inds
    Vector of row indices corresponding to the selected rows.

rows_view
    View of the selected rows of the full matrix `@view A[mask,:]`.

Notes
-----
The comparison is applied only to the column subset `A[:, cols]`,
but the returned rows correspond to the full matrix `A`.

Internally the row condition is evaluated with

    all(subA .>= thrT, dims=2)

(or the strict version if `include_equal=false`).
