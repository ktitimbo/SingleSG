...
# ==============================================================================
# ANGLE TRANSFORMATION: CQD_up_screen column 7 → collapsed/rotated angle
# ==============================================================================
"""
    transform_CQD_angle(CQD_up_screen, Icoils, ki)

For each coil index `ni = 1, ..., length(Icoils)`, extract column 7 (the angle
`x`) and column 5 (the longitudinal velocity `v_z`) from
`CQD_up_screen[:data][ni]`, then compute the CQD-transformed angle

    2 * atan( tan(x/2) * exp( -γₑ * ki * B * y_SG / v_z ) )

element-wise, where `B = TheoreticalSimulation.BvsI(Icoils[ni])`.

# Returns
- `OrderedDict{Int, Vector{Float64}}` mapping coil index `ni` to its vector of
  transformed angles (vectors may have different lengths across keys).
"""
function transform_CQD_angle(screen_data, Icoils, ki; mode::Symbol=:up)

    L_DRIFT = y_SG
    sign_exp = mode === :up ? -1 : mode === :dw ? +1 : error("mode must be :up or :dw")

    transformed_angles = OrderedDict{Int, Vector{Float64}}()

    for ni in 1:length(Icoils)
        x  = screen_data[:data][ni][:, 7]
        vz = screen_data[:data][ni][:, 5]
        B  = TheoreticalSimulation.BvsI(Icoils[ni])

        transformed_angles[ni] = 2 .* atan.(
            tan.(x ./ 2) .* exp.(sign_exp * abs.(γₑ) * ki * B * L_DRIFT ./ vz)
        )
    end
...
# ==============================================================================
# MEAN OF TRANSFORMED ANGLE vs COIL INDEX
# ==============================================================================
"""
    mean_CQD_angle(transformed_angles, Icoils)

Compute the mean of the CQD-transformed angle for each coil `ni`, returned as
a `Vector{Float64}` indexed by `ni = 1, ..., length(Icoils)` (so
`mean_angles[ni]` matches `transformed_angles[ni]`).

# Arguments
- `transformed_angles`: `OrderedDict{Int, Vector{Float64}}` from
  `transform_CQD_angle`.
- `Icoils`: used only to determine the number of coils / establish index order.

# Returns
- `Vector{Float64}`
"""
function mean_CQD_angle(transformed_angles::OrderedDict{Int, Vector{Float64}}, Icoils)

    mean_angles = Vector{Float64}(undef, length(Icoils))

    for ni in 1:length(Icoils)
        mean_angles[ni] = mean(transformed_angles[ni])
    end
...
# ==============================================================================
# HISTOGRAM COMPARISON: raw angle (col 7) vs CQD-transformed angle, per coil
# ==============================================================================
"""
    plot_CQD_angle_histogram(CQD_up_screen, transformed_angles, ni; bins=60)

Plot overlaid histograms comparing the raw angle distribution
`CQD_up_screen[:data][ni][:, 7]` against the CQD-transformed distribution
`transformed_angles[ni]`, for a single coil index `ni`.

# Arguments
- `CQD_up_screen`: original data container.
- `transformed_angles`: `OrderedDict{Int, Vector{Float64}}` from
  `transform_CQD_angle`.
- `ni`: coil index to plot.
- `bins`: number of histogram bins (default 60).

# Returns
- `Plots.Plot`
"""
function plot_CQD_angle_histogram(CQD_up_screen, transformed_angles, ni::Int)

    x_raw   = CQD_up_screen[:data][ni][:, 7]
    x_trans = transformed_angles[ni]
    @info Ic = "$(Int(1000*ICOILS[ni]))mA" θei = "$(round(mean(x_raw)/pi, digits=3))π" θef = "$(round(mean(x_trans)/pi, digits=3))π"

    p = histogram(x_raw/pi,
        bins  = TheoreticalSimulation.FreedmanDiaconisBins(x_raw),
        normalize = :pdf,
        ...
        xlabel = L"\theta_{e} / \pi",
        ylabel = "Counts",
        title  = L"$\theta_{e}$ distribution — %$(1000*ICOILS[ni])mA",
        legend = :topright
    )

    histogram!(p, x_trans/pi,
        bins  = TheoreticalSimulation.FreedmanDiaconisBins(x_trans),
        normalize=:pdf,
        label = "Final angle",
        color = :firebrick,
        alpha = 0.5
...
# --- Example usage ---
transformed_angles = transform_CQD_angle(CQD_up_screen, ICOILS, ki_ref; mode=:up)
mean_angles         = mean_CQD_angle(transformed_angles, ICOILS)
plot(ICOILS, mean_angles/pi,
    label=L"\langle \theta_{e}^{final}\rangle",
    xlabel="Current (A)",
    ylabel=L"\theta_{e} / \pi",
    xlims=(0.001,1.0)
)
plot!(xscale=:log10)
for i = 2:2:30
p = plot_CQD_angle_histogram(CQD_up_screen, transformed_angles, i)
end

transformed_angles = transform_CQD_angle(CQD_dw_screen, ICOILS, ki_ref; mode=:dw)
mean_angles         = mean_CQD_angle(transformed_angles, ICOILS)
plot(ICOILS, mean_angles/pi,
    label=L"\langle \theta_{e}^{final}\rangle",
    xlabel="Current (A)",
...
plot!(xscale=:log10)
for i = 2:2:30
p = plot_CQD_angle_histogram(CQD_dw_screen, transformed_angles, i)
end

# ==============================================================================
# EXECUTION: mean angle vs current + per-coil histograms, for up and down screens
# ==============================================================================
screen_configs = (
    (data=CQD_up_screen, mode=:up, tag="up", label=L"\langle \theta_{e}^{final} \rangle_{\uparrow}"),
    (data=CQD_dw_screen, mode=:dw, tag="dw", label=L"\langle \theta_{e}^{final} \rangle_{\downarrow}"),
)
...
for cfg in screen_configs
    ...
    angles_ni = transform_CQD_angle(cfg.data, ICOILS, ki_ref; mode=cfg.mode)
    mean_ni   = mean_CQD_angle(angles_ni, ICOILS)

    results[cfg.mode] = (angles=angles_ni, means=mean_ni)

    p_mean = plot(ICOILS, mean_ni ./ pi,
        label  = cfg.label,
        xlabel = "Current (A)",
        ylabel = L"\theta_{e} / \pi",
        xlims  = (0.001, 1.0),
    ...
    plot!(xscale=:log10)
    display(p_mean)
    savefig(p_mean, "mean_angle_vs_current_$(cfg.tag).png")

    for ni in 2:2:30
        p_hist = plot_CQD_angle_histogram(cfg.data, angles_ni, ni)
        display(p_hist)
        # savefig(p_hist, "angle_hist_$(cfg.tag)_ni$(ni).png")
    end
end

# Access later as, e.g.:
# results[:up].means
# results[:dw].angles[10]

1000*TheoreticalSimulation.BvsI(3e-3)
